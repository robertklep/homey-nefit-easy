...
var MD5 = function MD5(s, encoding) {
  return crypto.createHash('md5').update(s).digest(encoding);
};

// Magic number.
var MAGIC = new Buffer('58f18d70f667c9c79ef7de435bf0f9b1553bbb6e61816212ab80e5b0d351fbb1', 'hex');

function generateKey(magicKey, id_key_uuid, privatePassword) {
  return Buffer.concat([MD5(Buffer.concat([new Buffer(id_key_uuid, 'utf8'), magicKey])), MD5(Buffer.concat([magicKey, new Buffer(privatePassword, 'utf8')]))]);
}
...
Encryption.prototype.encrypt = function (data, type) {
  ...
  if (buffer.length % 16 !== 0) {
    buffer = Buffer.concat([buffer, new Buffer(16 - buffer.length % 16).fill(0)]);
  }
