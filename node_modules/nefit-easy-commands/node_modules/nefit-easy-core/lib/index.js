'use strict';

var Promise = require('bluebird');
var Queue = require('promise-queue');Queue.configure(Promise);
var debug = require('debug')('nefit-easy-core');
var rawDebug = require('debug')('nefit-easy-core:raw');
var HTTPParser = require('http-string-parser');
var XMPPClient = require('node-xmpp-client');
var Stanza = XMPPClient.Stanza;
var Encryption = require('./encryption');

// Default options for XMPP
var DEFAULT_OPTIONS = {
  host: 'wa2-mz36-qrmzh6.bosch.de',
  saslMechanism: 'DIGEST-MD5',
  requestTimeout: 30 * 1000,
  pingInterval: 30 * 1000
};

// Various prefixes used by Bosch.
var ACCESSKEY_PREFIX = 'Ct7ZR03b_';
var RRC_CONTACT_PREFIX = 'rrccontact_';
var RRC_GATEWAY_PREFIX = 'rrcgateway_';

var NefitEasyClient = module.exports = function NefitEasyClient(opts) {
  if (!(this instanceof NefitEasyClient)) return new NefitEasyClient(opts);

  // Merge options with defaults.
  this.opts = Object.assign({}, DEFAULT_OPTIONS, opts);

  // Generate some commonly used properties.
  var suffix = this.opts.serialNumber + '@' + this.opts.host;
  this.opts.jid = this.opts._from = RRC_CONTACT_PREFIX + suffix;
  this.opts._to = RRC_GATEWAY_PREFIX + suffix;

  // Queue that holds pending requests. This allows us to limit the number of
  // concurrent requests to 1, which is a requirement imposed by the backend.
  this.queue = new Queue(1, Infinity);

  // Initialize crypto stuff
  this.encryption = Encryption(this.opts.serialNumber, this.opts.accessKey, this.opts.password);

  // Create XMPP client.
  this.client = new XMPPClient({
    host: this.opts.host,
    jid: this.opts.jid,
    password: ACCESSKEY_PREFIX + this.opts.accessKey,
    preferredSaslMechanism: this.opts.saslMechanism,
    autostart: false,
    reconnect: true
  });

  // Pending GET requests.
  this.pending = {};
};

NefitEasyClient.prototype.ping = function () {
  var self = this;
  void function ping() {
    debug('sending ping');

    // Announce our presence.
    self.client.send('<presence/>');

    // Schedule the next ping.
    setTimeout(ping, self.opts.pingInterval).unref();
  }();
};

NefitEasyClient.prototype.connect = function () {
  var _this = this;

  // If not already connected/connecting, create a promise that is resolved
  // when a connection has been made (or rejected if an error occurred).
  if (!this.connectionPromise) {
    this.connectionPromise = new Promise(function (resolve, reject) {
      _this.client.once('online', function (r) {
        _this.jid = r.jid.toString();
        debug('online, jid = %s', _this.jid);
        _this.client.removeAllListeners('error');

        // Disable socket timeout and enable keepalives.
        _this.client.connection.socket.setTimeout(0);
        _this.client.connection.socket.setKeepAlive(true, 10000);

        // Send ping to backend to announce our presence.
        _this.ping();

        // Resolve the connection promise.
        return resolve(r);
      }).once('error', function (e) {
        debug('connection error', e);
        _this.client.removeAllListeners('online');
        return reject(e);
      }).connect();
    });
  }

  // Return the promise.
  return this.connectionPromise;
};

NefitEasyClient.prototype.end = function () {
  this.client.end();
};

NefitEasyClient.prototype.on = function () {
  return this.client.on.apply(this.client, arguments);
};

NefitEasyClient.prototype.send = function (message) {
  var _this2 = this;

  // Queue the request
  debug('queuing request');
  return this.queue.add(function () {
    // Send the message.
    debug('sending message');rawDebug(message);
    _this2.client.send(message);

    // Return a new promise that gets resolved once the response has been
    // received (or rejected).
    return new Promise(function (resolve, reject) {
      var removeListeners = function removeListeners() {
        clearTimeout(timer);
        _this2.client.removeListener('stanza', stanzaHandler);
        _this2.client.removeListener('error', errorHandler);
      };

      // Start timer for request timeouts.
      var timer = setTimeout(function () {
        removeListeners();
        return reject(new Error('REQUEST_TIMEOUT'));
      }, _this2.opts.requestTimeout);

      // Handler for incoming stanza messages.
      var stanzaHandler = function stanzaHandler(stanza) {
        // Process stanza.
        debug('received stanza');rawDebug(stanza.root().toString());

        if (stanza.is('message')) {
          // Meant for us?
          var to = stanza.attrs.to;
          if (to !== _this2.jid) {
            debug('..stanza addressed to %s, not to us.', to);
            return;
          }

          // Clear listeners.
          removeListeners();

          // Determine course of action based on stanza type.
          switch (stanza.attrs.type) {
            case 'error':
              var error = new Error('ERROR_RESPONSE');
              error.data = stanza.root();
              return reject(error);

            case 'chat':
              // Parse the response as if it were an HTTP response, and resolve it.
              return resolve(HTTPParser.parseResponse(stanza.root().getChild('body').getText().replace(/\n/g, '\r\n')));

            default:
              // Else, return the full stanza.
              return resolve(stanza);
          }
        }
      };

      // Error handler.
      var errorHandler = function errorHandler(e) {
        // Clear listeners.
        removeListeners();

        // Reject the request promise.
        return reject(e);
      };

      // Listen to the relevant client events.
      _this2.client.on('stanza', stanzaHandler);
      _this2.client.on('error', errorHandler);
    });
  });
};

NefitEasyClient.prototype.buildMessage = function (body) {
  return new Stanza('message', {
    from: this.opts._from,
    to: this.opts._to
  }).c('body').t(body).root().toString().replace(/\r/g, '&#13;\n');
};

NefitEasyClient.prototype.get = function (uri) {
  var _this3 = this;

  var message = this.buildMessage('GET ' + uri + ' HTTP/1.1\rUser-Agent: NefitEasy\r\r');

  // If we already have a request pending for this URI, send the message again but reuse the pending promise.
  if (uri in this.pending) {
    debug('using pending request for %s', uri);
    this.client.send(message);
  } else {
    this.pending[uri] = this.send(message).then(function (response) {
      if (response.statusCode !== '200') {
        var error = new Error('INVALID_RESPONSE');
        error.response = response;
        throw error;
      }

      // Decrypt message body and remove any padding.
      var decrypted = _this3.decrypt(response.body).replace(/\0*$/g, '');

      // Parse JSON responses.
      if (response.headers && response.headers['Content-Type'] === 'application/json') {
        try {
          decrypted = JSON.parse(decrypted);
        } catch (e) {
          throw e;
        }
      }
      return decrypted;
    }).finally(function () {
      debug('cleaning up for %s', uri);
      delete _this3.pending[uri];
    });
  }
  return this.pending[uri];
};

NefitEasyClient.prototype.put = function (uri, data) {
  // Encrypt the data.
  var encrypted = this.encrypt(typeof data === 'string' ? data : JSON.stringify(data));

  // Build the message.
  var message = this.buildMessage(['PUT ' + uri + ' HTTP/1.1', 'Content-Type: application/json', 'Content-Length: ' + encrypted.length, 'User-Agent: NefitEasy', '', encrypted].join('\r'));

  // Send it.
  return this.send(message).then(function (response) {
    if (Number(response.statusCode || 500) >= 300) {
      var error = new Error('INVALID_RESPONSE');
      error.response = response;
      throw error;
    }
    return response.body || { status: 'ok' };
  });
};

NefitEasyClient.prototype.encrypt = function (data, type) {
  return this.encryption.encrypt(data, type);
};

NefitEasyClient.prototype.decrypt = function (data, type) {
  return this.encryption.decrypt(data, type);
};